...
def test_sigmoid(a: float) -> None:
    ...
    * It is strictly increasing.
    ...
    if sigmoid(a) < 1.0:  # sigmoid could reach 1 due to float limit
...
def test_other(a: float, b: float) -> None:
    ...
    """
    if abs(a) > 1.0:  # to make the inverse of a not too large
        assert_close(inv_back(a, b), mul(neg(b), inv(a**2)))
        assert_close(log_back(a, b), mul(id(b), inv(a)))
